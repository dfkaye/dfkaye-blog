...
function shouldClose({ type, target, code }) {
  ...
  var closeOnEscapeOrSpace = /^Escape$/.test(code)
    || (/^Space$/.test(code) && !!closer);
...
function Underlay() {
  ...
  underlay.setAttribute("style", flat(`
    background: #ccc;
    height: 100%;
    opacity: .5;
    position: fixed;
...
// Show alert() on load.
// Note: top-level `await` is not supported in iOS Safari as 10 August 2021.

window.alert(`
This alert dialog is created by a polyfilled method for opening
the various window alerts (including confirm and prompt). Close
it by pressing the Escape or Enter key, or pressing the OK button.
`);
